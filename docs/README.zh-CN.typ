...
- https://gistd-cn.myriad-dreamin.com/typst/templates/blob/main/charged-ieee/template/main.typ
- https://gistd-cn.myriad-dreamin.com/Myriad-Dreamin/gistd/blob/main/samples/noto-sans-sc.typ?fonts=google-fonts:Noto+Sans+SC
...
- `fonts`: 额外注册的字体 provider。这个参数可以重复出现，每一个值都必须包含 provider，格式是 `provider:value`。
  ...
  - Google Fonts 的 axis 语法可以放在 family 后面，例如 `?fonts=google-fonts:Roboto:wght@400;700` 或 `?fonts=google-fonts:Roboto:ital,wght@0,400;1,700`。
