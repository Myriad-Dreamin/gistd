...
interface TypstRendererDriver extends TypstRenderer {
  renderer: RsTypstRenderer;
}

export function provideDomDoc<
  TBase extends GConstructor<TypstDocumentContext<InitDomDocArgs>>
>(Base: TBase): TBase & GConstructor<ITypstDomDocument> {
  return class DomDocument extends Base {
    ...
    /// Rust side of kernel.
    docKernel: IncrDomDocClient = undefined!;
    ...
    async mountDom(pixelPerPt: number | undefined) {
      ...
      this.resourceHeader = this.hookedElem.querySelector(
        ".typst-svg-resources"
      )!;

      this.docKernel = await this.plugin.renderer.mount_dom(
        (this.kModule as any)[kObject],
        this.hookedElem
      );
    ...
    retrieveDOMPages() {
      return Array.from(
        this.hookedElem.querySelectorAll(".typst-dom-page")
      ) as HTMLElement[];
    }
    ...
    preprocess(pages: { page: HTMLElement }[]) {
      ...
      if (this.maxPage && pages.length !== this.maxPage.val) {
        this.maxPage.val = pages.length;
      }
    }
    ...
    async rerender$dom() {
      ...
      const { x, y, width, height } = this.getDomViewport(
        domState.window,
        domState.boundingRect
      );

      let dirty = await this.docKernel.relayout(x, y, width, height);
      if (!dirty && this.prevRenderPage === this.partialRenderPage) {
        return;
      }

      const cancel = new TypstCancellationToken();
      this.prevRenderPage = undefined;
    ...
    async doRender$dom(ctx: TypstCancellationToken) {
      ...
      const renderPage = async (i: number) => {
        ...
        const wScale =
          (browserBBox.width
            ? Number.parseFloat(page.getAttribute("data-width")!) /
              browserBBox.width
            : 1) * this.domScale;
        const hScale =
          (browserBBox.height
            ? Number.parseFloat(page.getAttribute("data-height")!) /
              browserBBox.height
            : 1) * this.domScale;
        v.x *= wScale;
        v.y *= hScale;
        ...
        for (let idx = 0; idx < pages.length; ++idx) {
          ...
          const pageInWindow =
            this.previewMode === PreviewMode.Slide
              ? idx === this.partialRenderPage
              : pages[idx].inWindow;

          if (pageInWindow === inWindow) {
